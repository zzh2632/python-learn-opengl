...
def main():
    ...
    with open("material.vs", "rb") as file:
        lightingVertexShaderSource = file.read()
    with open("material.fs", "rb") as file:
    ...
    while not glfw.window_should_close(window):  # 检查一次GLFW是否被要求退出
        ...
        # 传入材质
        glUniform3f(glGetUniformLocation(lightingShader, "material.ambient"), 1.0, 0.5, 0.31)
        glUniform3f(glGetUniformLocation(lightingShader, "material.diffuse"), 1.0, 0.5, 0.31)
        glUniform3f(glGetUniformLocation(lightingShader, "material.specular"), 0.5, 0.5, 0.5)
        ...
        diffuseColor = lightColor * vec3(0.5)
        ambientColor = diffuseColor * vec3(0.2)
        glUniform3fv(glGetUniformLocation(lightingShader, "light.ambient"), 1, GL_FALSE, ambientColor)
        glUniform3fv(glGetUniformLocation(lightingShader, "light.diffuse"), 1, GL_FALSE, diffuseColor)
